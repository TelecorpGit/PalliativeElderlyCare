...
import com.telecorp.teledev.pec.File_Confix_Data;
import com.telecorp.teledev.pec.R;

import java.util.ArrayList;
import java.util.List;


public class DeviceAdapter_BluetoothFastBle extends BaseAdapter {

    private Context context;
    private List<BleDevice> bleDeviceList;

    public DeviceAdapter_BluetoothFastBle(Context context) {
    ...
    public void addDevice(BleDevice bleDevice) {
        ...
        try {
            boolean isConnected = BleManager.getInstance().isConnected(bleDevice);
                    ...
                    char name_[]= bleDevice.getName().toCharArray();

            if(name_.length>5){
                String name_d=   String.valueOf(name_[0])
                        +   String.valueOf(name_[1])
                        +   String.valueOf(name_[2])
                        +   String.valueOf(name_[3])
                        +   String.valueOf(name_[4])
                        +   String.valueOf(name_[5]);
                if(name_d.equals(File_Confix_Data.name_bluetooht) ){
                    if(isConnected){
